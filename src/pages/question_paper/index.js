...
const QuestionPaper = (props) => {
	...
	useEffect(() => {
		...
		axios
			.get('/users/' + authState.userKey + '/submittedResponses.json' + queryParams)
			.then((response) => {
				if (response.data) {
					const queryParams3 = `?auth=${authState.token}&orderBy="paperId"&equalTo="${qkey}"`
					axios
						.get('/users/' + authState.userKey + '/submittedResponses.json' + queryParams3)
						.then((response) => {
		...
		if (onSubmitHandler()) {
			swal({
				...
				if (willSubmit) {
					...
					axios
						.post('/responses.json?auth=' + authState.token, questionPaperResponse)
						.then((response) => {
