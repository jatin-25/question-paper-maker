...
const MCQuestion = (props) => {
    ...
    const [answersArr, setAnswersArr] = useState(props.showEditButton || !(props.answer) ? ['Not Marked'] : props.answer);
    ...
    let optionsComp = props?.optionsList?.map((option, index) => {
        return (
            <div key={index} className="Option">
                <input type="checkbox" name={props.qkey} onChange={() => updateAnswersArrHandler(option)} disabled={props.pageOnWhichRendered !== 'questionPaper'}></input>
                <span>{option}</span>
            </div>
        )
    });
    ...
    return (
        <div className="Question">
            <p style={{ marginBottom: "5px", display: "inline-block" }}>Ques {props?.qkey + 1}: {props?.question || ""}</p>
            {optionsComp}
            {selectedAnswer}
            {props?.pageOnWhichRendered === 'newPaper' && <div className="EditButtonContent">
                <BiIcons.BiEdit onClick={() => props.onEditHandler({ idx: props?.qkey, type: "MultipleChoiceQuestion" })} className="EditIcon" />
